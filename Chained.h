...
template<>
uint32_t ChainedMap<std::string, uint32_t>::defaultGetReturn() {
    return -1;
}
template<>
Product* ChainedMap<uint32_t, Product*>::defaultGetReturn(){
    return nullptr;
}


template<>
uint32_t ChainedMap<std::string, uint32_t>::hash(std::string k){
    return hasher(k)%hashmapSize;
}
template<>
uint32_t ChainedMap<uint32_t, Product*>::hash(uint32_t k){
...
template<>
void ChainedMap<std::string, uint32_t>::printKeyValuePair(ChainedMap::KeyValuePair kvp) {
    std::cout << "<" << kvp.key << ", " << kvp.value << "> ";
}

template<>
void ChainedMap<uint32_t, Product*>::printKeyValuePair(ChainedMap::KeyValuePair kvp) {
